...
module.exports = (componentName, routeName) => {
  ...
  let dir = './src/components/' + nameList.name;
  fs.mkdirSync(dir);
  ...
  const replaceEntries = [
    {
      ...
      from: /(import.*\n(?!.*import.*))/,
      to: `$1import ${nameList.name}Reducer from '../components/${nameList.name}/reducer'\n`,
    },
    {
      files: './src/app/reducers.js',
    ...
    },
    {
      ...
      from: /(import.*\n(?!.*import.*))/,
      to: `$1import ${nameList.pascalCasedName} from '../components/${nameList.name}'\n`,
    },
    {
      files: './src/app/routes.js',
